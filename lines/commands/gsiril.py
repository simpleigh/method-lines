    ...
    def execute(self):
        ...
        with open(filename, 'w') as file:
            ...
            if self.composition.configs.has_config('calls'):
                for name, call in six.iteritems(self.composition.configs.calls):
                    output('c_{name} = +{call};'.format(name=name, call=call))
                output()

            notations = []
            for lead in self.composition.leads:
                if lead.call_object is not None:
                    notations.append('m_{}_lead'.format(lead.method_name))
                    notations.append('c_{}'.format(lead.call_symbol))
                else:
                    notations.append('m_{}_plain'.format(lead.method_name))
